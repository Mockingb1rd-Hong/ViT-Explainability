...
def compute_equivalent_attention(cam, gradients):
    """
    ...
    '''
    threshold = 0.8
    # print("After row normalization:", A)
    # print("Size of A:", A.size())
    # Apply thresholding to corner elements of each row
    A_thresholded = A.clone()
    for i in range(A.size(0)):
        # Check the first element of the row
        if A_thresholded[i, 0] < threshold:
            A_thresholded[i, 0] = 0.0
        
        # Check the last element of the row
        if A_thresholded[i, -1] < threshold:
            A_thresholded[i, -1] = 0.0
    '''
    return A
...
def update_relevance_map_sum(R, equivalent_attention, one_hot, Y, Y_prime, alpha_sum):
    ...
    R = alpha * R + beta * R_Y_prime_X.unsqueeze(0)
    
    return R, current_alpha

def update_relevance_map_prev(R, equivalent_attention, one_hot, Y, Y_prime):
    """
    Updates the relevance map.
    ...
    epsilon = 0.1  # Small bias towards output relevance
    alpha = (Y_grad * Y).sum(dim=-1) / ((Y_grad * Y).sum(dim=-1) + (Y_prime_grad * Y_prime).sum(dim=-1))
    beta = 1 - alpha

    # Adjust alpha and beta
    alpha = alpha / (1 + epsilon)
    beta = (beta + epsilon) / (1 + epsilon)
...
def interpret_text(texts, model, image, device, start_layer_text=-1):
    batch_size = texts.shape[0]
    images = image.repeat(batch_size, 1, 1, 1)
...
def interpret_image_weighted_prev(image, texts, model, device, start_layer=-1):
    ...
    for i, blk in enumerate(image_attn_blocks):
        ...
        Y_prime = blk.output  # Assuming Y_prime is the output of the current layer (this needs to be stored/accessible)
        # Print sizes of variables
        R = update_relevance_map_prev(R, equivalent_attention, one_hot, Y, Y_prime)
    # Extract the relevance of the image tokens
    image_relevance = R[:, 0, 1:]
    return image_relevance